...
```text
QEMUがRISC-V仮想マシンを起動する
　↓
OpenSBIというファームウェアが起動する
　↓
OpenSBIが自作OSのbootへジャンプする
　↓
bootでスタックポインタを設定する
　↓
カーネルのkernel_mainを呼び出す
　↓
main関数を呼び出す
　↓
printf関数で文字を表示する
...
今回のカーネルでは、まず `boot` という入口から処理が始まります。そこでスタックポインタを設定し、その後カーネルの `kernel_main` を呼び出します。`kernel_main` の中で、ようやくC言語で書いた `main` 関数を呼び出します。
